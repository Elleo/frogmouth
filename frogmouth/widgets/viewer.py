...
class Viewer(VerticalScroll, can_focus=True, can_focus_children=True):
    ...
    async def _local_load(self, location: Path, remember: bool = True) -> None:
        ...
        """
        # Convert document to mark down
        try:
            pd = pandoc.read(file=location.as_posix())
            md = pandoc.write(pd, format="markdown")
            self.document.update(md)
            self._post_load(location, remember)
        except RuntimeError:
            self.app.push_screen(
                ErrorDialog(
                    "Pandoc not found",
                    "Please install pandoc to load documents.",
                )
            )
        except:
            self.app.push_screen(
